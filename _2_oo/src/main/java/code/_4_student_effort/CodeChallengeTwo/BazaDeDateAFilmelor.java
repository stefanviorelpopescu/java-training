...
public class BazaDeDateAFilmelor {
    ...
    public static void StudiosWithMoreThan2MoviesPublished(){
        for(Studio studio:getData())
        {if(studio.getFilme().length>2)
            System.out.println(studio.getNume());
    }}
    public static void MoviesWithActorsWith2Oscars(){
        for(Studio studio:getData())
            for(Film filme:studio.getFilme())
                for(Actor actor: filme.getActori())
                    if(actor.getNume().equalsIgnoreCase("actor cu 2 oscaruri"))
                        System.out.println(studio.getNume());
    }
    public static void MoviesWithActorOver50YearsOld(){
        for(Studio studio:getData())
            for(Film filme:studio.getFilme())
                for(Actor actor: filme.getActori())
    ...
    public static void main(String[] argv){
        StudiosWithMoreThan2MoviesPublished();
        MoviesWithActorsWith2Oscars();
        MoviesWithActorOver50YearsOld();
}
}
