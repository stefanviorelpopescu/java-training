...
public class Main {
  ...
  public static void MovieDatabase() {
    ...
    for (Studio studio : studioDatabase) {
      if (studio.getFilme().length > 2) System.out.println("Studio cu mai mult de doua filme" + studio.getNume());
      for (Film film : studio.getFilme())
        for (Actor actor : film.getActori())
          if (actor.getNume().equals("actor cu 2 oscaruri"))
            System.out.println("Actorul joaca la studioul: " + studio.getNume());
    }
    for (Studio studio : studioDatabase)
      for (Film film : studio.getFilme())
        for (Actor actor : film.getActori())
          if (actor.getVarsta() > 50) {
            System.out.println("Film cu actor care are peste 50 de ani: " + film.getNume());
          }
  }
