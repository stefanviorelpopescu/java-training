...
public class Main {
    public static void main(String[] args) {
        ...
        System.out.println("\nTask2: ");
        for (int i = 0; i < studioDataBase.length; ++i) {
            Film[] filme_curr_studio = studioDataBase[i].getFilme();
            for (int j = 0; j < filme_curr_studio.length; ++j) {
                Actor[] actori_curr_film = filme_curr_studio[j].getActori();
                for (int k = 0; k < actori_curr_film.length; ++k) {
                    if (actori_curr_film[k].getNume().equals("actor cu 2 oscaruri")) {
                        System.out.println(studioDataBase[i].getNume() + "  ");
                        break;
                    }
                }
        ...
        for (int i = 0; i < studioDataBase.length; ++i) {
            ...
            for (int j = 0; j < filme_curr_studio.length; ++j) {
                Actor[] actori_curr_film = filme_curr_studio[j].getActori();
                for (int k = 0; k < actori_curr_film.length; ++k) {
                    if (actori_curr_film[k].getVarsta() >= 50) {
                        System.out.println(filme_curr_studio[j].getNume() + "  ");
                        break;
                    }
                }
            }
